...
#> 
   ...
   Write-Host "Creating the AAD application (restricted-scopes-v2)"
   # Get a 2 years application key for the service Application
   $pw = ComputePassword
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInYears 2 -pw $pw
   $serviceAppKey = $pw
   $serviceAadApplication = New-AzureADApplication -DisplayName "restricted-scopes-v2" `
                                                   ...
                                                   -IdentifierUris "https://$tenantName/restricted-scopes-v2" `
   ...
   Write-Host "Done creating the service application (restricted-scopes-v2)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>restricted-scopes-v2</a></td></tr>" -Path createdApps.html
   ...
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "Microsoft Graph" `
                                                -requiredDelegatedPermissions "openid|email|profile|offline_access|User.Read|Group.Read.All|User.ReadBasic.All" `
   ...
   $configFile = $pwd.Path + "\..\GroupManager\Web.Config"
